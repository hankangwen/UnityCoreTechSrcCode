...
public class VirtualStickUI : MonoBehaviour 
{
	...
	{		
        ...
        Game.Ctrl.UIGuideCtrl.Instance.AddUiGuideEventBtn(btnSelf.gameObject);
	}
    ...
    private void PlayerAdMove(Vector3 mvDir) {
        Iselfplayer player = PlayerManager.Instance.LocalPlayer;
        if (Game.Skill.BuffManager.Instance.isHaveStopBuff(player.GameObjGUID))
        {
            return;
        }
        if (player.FSM == null)
