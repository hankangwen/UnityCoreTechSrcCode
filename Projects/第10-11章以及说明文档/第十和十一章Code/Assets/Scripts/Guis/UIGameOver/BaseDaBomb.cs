        ...
        if (isBombBase)
        {
            ...
            if (move == transform.position && Time.time - startTime >= 1f)
            {
                ...
                if (move == posDest)
                {
                    ...
                    ani.enabled = true;
                     */
					//soul = Game.AudioManager.Instance.PlayEffectAudio(Resources.Load(AudioDefine.PATH_GAME_OVER_FAILE) as AudioClip);
                    ...
                    soul = Game.AudioManager.Instance.PlayEffectAudio(clip);
                }
                else
                {
					//soul = Game.AudioManager.Instance.PlayEffectAudio(Resources.Load(AudioDefine.PATH_GAME_OVER_VICTORY) as AudioClip);
